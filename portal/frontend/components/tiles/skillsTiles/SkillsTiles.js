...
export class SkillsTiles {
    ...
    setView() {
        appendChildren(this.view, [
            ...
            addEvent(addClasses(createSVGButton('portal/frontend/assets/icons/Edit.svg'), 'skillsTiles_updateButton'), () => { const close = this.parentProps.displayBox(new UpdateSkills(this.parentProps, this.skill, () => { close() }, () => { this.refresh() }).view) }),
            addEvent(addClasses(createSVGButton('portal/frontend/assets/icons/Delete.svg'), 'skillsTiles_deleteButton'), async () => { 
                // await deleteSkillsData(this.skill.id); this.refresh();
             })
        ])
    }
}