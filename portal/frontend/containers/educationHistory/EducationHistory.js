...
export class EducationHistory {
    ...
    async fetch() {
        ...
        await fetch( 'service/backend/controllers/GET/getEducation.php' )
    ...
    setView() {
        appendChildren(this.view, [
            ...
            addEvent(addClasses(createSVGButton('portal/frontend/assets/icons/Plus.svg'), 'resumeView_addButton'), () => { const close = this.parentProps.displayBox(new AddEducation(this.parentProps, () => { close() }, () => { detachChildren(this.view); this.fetch(); }).view) }),
        ])
    }
}