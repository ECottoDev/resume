...
import { BRIDGE_SERVICE } from "./constants.js";
...
export function getHttpRequest(params = {}, route = BRIDGE_SERVICE) {
    try {
        const searchParams = new URLSearchParams(params);
        return fetch(route + (!isObjectEmpty(params) ? `?${searchParams.toString()}` : ''), { method: 'GET' })
            .then(response => {
                if (response.ok) {
...
export function postHttpRequest(params, statusCallBack = false, route = BRIDGE_SERVICE) {
    return new Promise(resolve => {
        const formData = new FormData();
        formData.append('params', JSON.stringify(params, jsonStringifyReplacer));
        const xhr = new XMLHttpRequest();
        xhr.onreadystatechange = function () {
            if (this.readyState === DONE_STATE && this.status === OK_STATUS_CODE) {
                try {
                    const response = JSON.parse(this.responseText);
                    response === UNAUTHORIZED_STATUS_CODE ? redirect('./index.html') : resolve(response);
                } catch (error) {
                    console.error(error);
                    resolve(false);
                }
            } else if (this.status > OK_STATUS_CODE) {
                resolve(false);
            }
        };
        statusCallBack && xhr.upload.addEventListener('progress', function (e) {
            if (e.lengthComputable) {
                statusCallBack(Math.round((e.loaded * 100) / e.total));
            }
        }, false);
